...
# Get your current location with your IP address
get_current_city() {
  local city
  local location_data
  location_data=$(curl -fsS --max-time 10 "https://ipinfo.io/json" 2>/dev/null)
  ...
  # Fallback: ipapi.co
  city=$(curl -fsS --max-time 10 "https://ipapi.co/json" 2>/dev/null | grep -o '"city"[[:space:]]*:[[:space:]]*"[^"]*"' | cut -d'"' -f4)
  if [[ -n "$city" ]]; then
    echo "$city"
    return 0
  fi
  ...
  city=$(curl -fsS --max-time 10 "https://ipwho.is/" 2>/dev/null | grep -o '"city"[[:space:]]*:[[:space:]]*"[^"]*"' | cut -d'"' -f4)
  if [[ -n "$city" ]]; then
    echo "$city"
    return 0
  fi
...
# If city is empty, use manual setting
...
if [[ $cacheage -gt 1740 ]] || [[ ! -s "$cachedir/$cachefile" ]]; then
  ...
  mapfile -t sdata < <(curl -fsS --max-time 10 "https://wttr.in/${encoded_city}?format=%25l%0A%25C%0A%25t&lang=en" 2>/dev/null || true)
  if [[ ${#sdata[@]} -ge 3 ]]; then
  ...
  loc=$(curl -fsS --max-time 10 "https://wttr.in/${encoded_city}?format=%25l&lang=en" 2>/dev/null || true)
  cond_only=$(curl -fsS --max-time 10 "https://wttr.in/${encoded_city}?format=%25C&lang=en" 2>/dev/null || true)
  temp_only=$(curl -fsS --max-time 10 "https://wttr.in/${encoded_city}?format=%25t" 2>/dev/null || true)
  if [[ -n "$loc" ]] && [[ -n "$cond_only" ]] && [[ -n "$temp_only" ]]; then
      ...
      mapfile -t data < <(curl -fsS --max-time 10 "$url" 2>/dev/null || true)
